...
class UserPolicy:
    ...
    def daily_decision(self, t):
        ...
        def slicerp(t0, a30):
            ...
            cut2 = cut[cut['date'] <= a30]
            return cut2

        def slicerp_sub(t0, a30):
        ...
        def prom(t0, a30):
            ...
            prom_data = pd.get_dummies(slicerp(t0, a30)['promotion_type'])
        ...
        def prom_sub(t0, a30):
            sku_ids = [i for i in range(1, 1001)]
            shell = pd.DataFrame(index = sku_ids, columns = [1,4,6,10]).fillna(0)
            prom_data = pd.get_dummies(slicerp_sub(t0, a30)['promotion_type'])
            non_repeated = list(set(prom_data.index))
            for i in non_repeated:
                a = prom_data[prom_data.index == i][1].sum()
                b = prom_data[prom_data.index == i][4].sum()
        ...
        def preprocess(t0, t7, t14, t30, a30, dc_id):
            ...
            a_30_raw = slicerx(t0, a30, dc_id)
            discount = a_30_raw.groupby(a_30_raw.index)['discount'].mean()
            original = a_30_raw.groupby(a_30_raw.index)['original_price'].mean()
            joined = pd.concat([p_7, p_14, p_30, discount, original], axis=1)
            promotions = prom(t0, a30)
            join_prom = pd.concat([joined, promotions], axis = 1)
            join_prom.columns = ['p_7', 'p_14','p_30','discount', 'original', '1', '4', '6', '10']
            a_30 = a_30_raw.groupby(a_30_raw.index)['quantity'].sum()
            join_prom = join_prom.fillna(0)
            a_30 = a_30.fillna(0)
            return [join_prom,a_30]

        def preprocess_sub(t0, t7, t14, t30, a30, dc_id):
            p_7_raw = slicerx(t7, t0, dc_id)
            p_7 = p_7_raw.groupby(p_7_raw.index)['quantity'].sum()
            p_14_raw = slicerx(t14, t0, dc_id)
            p_14 = p_14_raw.groupby(p_14_raw.index)['quantity'].sum()
